...
import { deleteCar, getCarById } from '../../api';
...
export default function ModalConfirm({ idItem, getCarList }) {
    ...
    const handleDelete = async (id) => {
        const message = await deleteCar(id);
        if (message.statusText === 'No Content') {
            setShowSuccess(true);
        }
    };
    ...
    return (
        <div>
            <ButtonCustom onClick={handleOpen} nameButton="Xóa" className="btn btn-danger fix_width_96" />
            <Modal
                open={open}
                onClose={handleClose}
            ...
            >
                {!showSuccess ? <Box sx={style}>
                    ...
                    <p className={styles.content}>
                        ...
                        <span className={styles.content_name}> {carItem.name}</span>
                    </p>
                    <p>Sản phẩm sẽ bị <span className={styles.content_name}> xóa vĩnh viễn</span>
                    </p>
